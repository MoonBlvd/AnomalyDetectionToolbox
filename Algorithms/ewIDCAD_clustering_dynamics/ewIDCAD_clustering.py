...
beta = 2 #
start = 20 # size of initial set is 3
stablization = 10
tmp = 0.5
...
if __name__ == '__main__':
    ...
    #data_file_name = 'anomalous_data_1.csv'
    ...
    #file_path = '../../Nan_Traffic_Simulator/anomalous_data/'
    #data_file_name = 'anomalous_18D_5D_1.csv'
    ...
    clusters = []
    init_type = 'normal'
    clusters.append(cluster(init_samples, alpha, beta, _lambda, mean, cov_inv, gamma, init_type,1))
    anomalies = None
    cum_anomalies = 2
    j = 0
    tmp_index = None
    ...
    k = 0
    curr_cluster = cluster(init_samples, alpha, beta, _lambda, mean, cov_inv, gamma, init_type,0)
    ...
    for i in range(start, num_seqs):
        ...
        for j in range(len(update_clusters)):
            #save_flag = True
            update_clusters[j].update(new_instance, i)
            #save_flag = False
        curr_cluster.update(new_instance,i)
        ...
        if B != None and len(B) > num_sensors+1:
            ...
            if two_separated(curr_cluster, clusters):
                ...
                clusters.append(cluster(B, alpha, beta, _lambda, mean, cov_inv, gamma, init_type, len(clusters)+1))
                B = None
