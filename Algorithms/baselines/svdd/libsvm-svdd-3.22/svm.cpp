		   ...
		   SolutionInfo* si, int shrinking)
{
	...
	while(iter < max_iter)
	{
		...
		if(y[i]!=y[j]) // if i and j are two instances with different label
		{
			...
			if(diff > 0)
			{
				if(alpha[j] < 0) // constraint alpha > 0
				{
					alpha[j] = 0;
					alpha[i] = diff;
				}
			...
			{
				if(alpha[i] < 0) // constraint alpha > 0
				{
					alpha[i] = 0;
					alpha[j] = -diff;
				}
		...
		else // if i and j are two instances with identical label
		{
			double quad_coef = QD[i]+QD[j]-2*Q_i[j];
			if (quad_coef <= 0)
				quad_coef = TAU;
