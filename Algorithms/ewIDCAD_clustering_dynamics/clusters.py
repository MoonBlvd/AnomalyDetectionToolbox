...
class cluster():
    def __init__(self, elements, alpha, beta, _lambda, mean, cov_inv, gamma, cluster_type,cluster_num):
        ...
        self.gamma = gamma
        self.num = cluster_num # the number of the current cluster
    ...
    def update(self, x, index):
        ...
        diff = (x-self.mean) # make it to be 2D array. 1*2
        ...
        '''
        meanfile = open('cluster'+str(self.num)+'_mean.csv', 'a')
        writer = csv.writer(meanfile, delimiter = ',')
        writer.writerow(np.append(index,self.mean))
        meanfile.close
        
        cov_inv_file = open('cluster'+str(self.num)+'_cov_inv.csv', 'a')
